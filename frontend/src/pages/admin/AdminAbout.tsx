...
export const AdminAbout: React.FC = () => {
  ...
  const [advisorFiles, setAdvisorFiles] = useState<{[key: number]: File}>({});
    ...
    fetch('/api/settings/about')
      ...
      .then(data => {
        if (data.historyImage) {
          setHistoryImagePreview(data.historyImage.startsWith('http') ? data.historyImage : `${data.historyImage}`);
        }
        if (data.advisors) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const cleanAdvisors = advisors.map(m => {
        const { imageFile, ...rest } = m;
        return rest;
      });
      formData.append('advisors', JSON.stringify(cleanAdvisors));
      
      Object.keys(advisorFiles).forEach(indexStr => {
        formData.append(`advisorImage_${indexStr}`, advisorFiles[parseInt(indexStr)]);
      });
  ...
  const openModal = (index: number | null = null) => {
    if (index !== null) {
      ...
      setMemberForm({
        name: m.name, role: m.role, period: m.period, description: m.description, socialLink: m.socialLink || '', 
  ...
  const openAdvisorModal = (index: number | null = null) => {
    if (index !== null) {
      const m = advisors[index];
      setMemberForm({
        name: m.name, role: m.role, period: m.period, description: m.description, socialLink: m.socialLink || '', 
        imagePreview: m.image || '', file: null
      });
  ...
  const saveAdvisor = () => {
    ...
    let targetIndex = editingAdvisorIndex;
    if (targetIndex !== null) {
      newAdvisors[targetIndex] = advisorData;
    } else {
      newAdvisors.push(advisorData);
      targetIndex = newAdvisors.length - 1;
    }

    if (memberForm.file) {
      setAdvisorFiles(prev => ({ ...prev, [targetIndex as number]: memberForm.file as File }));
    }
  ...
  const deleteAdvisor = (index: number) => {
    if(confirm('Yakin ingin menghapus pembina ini?')) {
      ...
      const newFiles = { ...advisorFiles };
      delete newFiles[index];
      setAdvisorFiles(newFiles);
    }
  };
    ...
    <div className="flex flex-col min-h-screen bg-background py-8 relative">
      <div className="max-w-container-max mx-auto px-4 sm:px-6 md:px-gutter w-full">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          {/* Kelola Dewan Pembina */}
          <div className="lg:col-span-8 lg:col-start-5 mt-8">
            <div className="bg-surface-lowest rounded-2xl border border-border-subtle overflow-hidden shadow-level1">
              <div className="p-6 border-b border-border-subtle flex justify-between items-center bg-surface">
                <div>
                  <h3 className="text-lg font-display font-bold text-primary flex items-center gap-2">
                ...
                >
                  <Plus className="w-4 h-4" />
                  <span className="hidden sm:inline">Tambah Pembina</span>
                </button>
              </div>
                  ...
                  <tbody className="divide-y divide-border-subtle">
                    {advisors.map((advisor, idx) => (
                      <tr key={idx} className="hover:bg-surface/50 transition-colors">
                        <td className="px-6 py-4">
                          {advisor.image ? (
                            <img src={advisor.image} alt={advisor.name} className="w-10 h-10 rounded-full object-cover border border-border-subtle" />
                          ) : (
                            <div className="w-10 h-10 rounded-full bg-surface-variant flex items-center justify-center text-xs text-outline">No</div>
                          )}
                        </td>
                        <td className="px-6 py-4">
                          <p className="text-sm font-semibold text-primary">{advisor.name}</p>
                          <p className="text-xs text-outline">{advisor.period}</p>
                        </td>
                        <td className="px-6 py-4">
                          <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-primary-fixed text-primary">
                            {advisor.role}
                        ...
                        <td className="px-6 py-4 text-right">
                          <div className="flex items-center justify-end gap-2">
                            <button onClick={() => openAdvisorModal(idx)} className="p-1.5 text-outline hover:text-primary hover:bg-primary-fixed rounded transition-colors" title="Edit">
                              <Edit2 className="w-4 h-4" />
                            </button>
                            <button onClick={() => deleteAdvisor(idx)} className="p-1.5 text-outline hover:text-error hover:bg-error/10 rounded transition-colors" title="Hapus">
                              <Trash2 className="w-4 h-4" />
                            </button>
                          </div>
                        </td>
      ...
      {isAdvisorModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-surface-lowest rounded-2xl border border-border-subtle p-6 max-w-md w-full shadow-2xl relative">
            <button onClick={() => setIsAdvisorModalOpen(false)} className="absolute top-4 right-4 p-1 text-outline hover:text-primary">
              <X className="w-5 h-5" />
            </button>
            <h3 className="text-lg font-display font-bold text-primary mb-4">
              {editingAdvisorIndex !== null ? 'Edit Pembina' : 'Tambah Pembina'}
            </h3>
            ...
            <div className="space-y-4 text-sm">
              ...
              <div>
                <label className="block text-xs font-semibold text-on-surface mb-1">Foto Pembina</label>
                <div className="flex items-center gap-4">
                  {memberForm.imagePreview ? (
                    <img src={memberForm.imagePreview} alt="Preview" className="w-12 h-12 rounded-full object-cover border" />
                  ) : (
            ...
            <div className="mt-6 flex justify-end gap-3">
              <button onClick={() => setIsAdvisorModalOpen(false)} className="px-4 py-2 text-sm font-semibold text-on-surface-variant hover:text-primary transition-colors">Batal</button>
              <button onClick={saveAdvisor} className="px-4 py-2 bg-primary text-white text-sm font-display font-bold rounded-lg hover:bg-primary-container transition-colors">Simpan</button>
            </div>
          </div>
        </div>
      )}
